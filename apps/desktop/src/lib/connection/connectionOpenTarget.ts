...
export type QuickConnectionOpenTarget = { kind: "mq-admin" } | { kind: "nacos-admin" } | { kind: "ssh-workbench" } | { kind: "etcd" } | { kind: "zookeeper" } | { kind: "query"; database: string };
...
export function quickConnectionOpenTarget(connection: Pick<ConnectionConfig, "db_type" | "database">, databaseOptions: string[] = []): QuickConnectionOpenTarget {
  ...
  if (connection.db_type === "ssh") {
    return { kind: "ssh-workbench" };
  }
