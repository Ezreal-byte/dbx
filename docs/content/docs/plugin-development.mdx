...
Below the connection tree, icons wrap into rows with vertical scrolling. Automatic height fits the required rows up to five, based on the available columns. Drag the divider to save a custom height; double-click it to restore automatic height. Escape, window blur, or releasing outside the sorting area cancels sorting. Failed saves show an error and roll back.
